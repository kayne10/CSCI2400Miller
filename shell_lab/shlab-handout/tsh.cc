...
int builtin_cmd(char **argv) 
{
	...
	//ignores command if & is the first char
...
void do_bgfg(char **argv) 
{
	// The id could be a PID or a JID
	struct job_t *job;
	char *id = argv[1];
	int jid;	
...
void waitfg(pid_t pid)
{
	// just wait until the process is out of foreground
...
void sigchld_handler(int sig) 
{
	...
	// Waiting for and handling all of the child processes according to their status
...
{  
	...
	// send fg job and related process group signal
	if (pid != 0) {  
